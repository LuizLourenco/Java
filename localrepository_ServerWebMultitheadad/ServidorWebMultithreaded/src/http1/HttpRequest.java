...
final class HttpRequest implements Runnable{
	...
	private void processRequest () throws Exception{
		...
		if (metodo.equals("GET")){
			...
			//Verifica se o arquivo é RESTRITO, forma colocadas apenas
			...
			if((ehRestrito) && (ehAutenticada) == false){
				...
				linhaContentType = "WWW-Authenticate: Basic realm=\"RESTRITO\""+CRLF;
				msgHtml = "<HTML><HEAD><TITLE> Acesso Nao Autorizado " + "</TITLE></HEAD>" + "<BODY> Acesso Nao Autorizado </BODY></HTML>";
				existeArq = false;
			}else{
				if(existeArq){
			...
			if((i = br.read(buffer)) > 0)
				...
				corpoPost = corpoPost + (new String(buffer,0,i) + "<BR>");
					...
					log = log + corpo;
					msgHtml = "<HTML><HEAD><TITLE> MENSAGEM POST</TITLE></HEAD>" + "<BODY> MENSAGEM ENVIADA PELO POST: </BR>"+corpoPost+"</BODY></HTML>";
					existeArq = false;
					}
			if(existeArq){
				sendBytes(fis, dos);
		...
		private static String contentType(String arquivo){
			if(arquivo.endsWith(".htm")||arquivo.endsWith(".html")||arquivo.endsWith(".txt")) 
				return "text/html";
